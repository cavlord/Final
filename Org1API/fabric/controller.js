...
//Create Data for Vaccnation
exports.createDataVac = async (req, res, next) => {
  try {
    // connect to network
    ...
    const data = req.body;
    let txn = await contract.createTransaction("createVaccineAsset");
    txn.setTransient({
      remark: data.remark,
    });
    txn.submit(
...
//Update data Vaccination
exports.updateDataVac = async (req, res, next) => {
  try {
    // connect to network
    const network = await connectToNetwork();
    // Get the contract from the network.
